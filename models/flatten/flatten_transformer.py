...
def flatten_swin_tiny(num_classes=1000, pretrained=False):
    if pretrained:
        model = FLattenSwinTransformer(  img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                                         embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                                         window_size=56, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                                         drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
                                         norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False,
                                         focusing_factor=3, kernel_size=5, attn_type='LLSS')
        url = 'https://cloud.tsinghua.edu.cn/seafhttp/files/0a326d30-1a1e-437b-9c04-cc1b639c4194/ckpt_swin_t.pth'
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        # checkpoint = torch.load("NAT_FAT/ImageNet_pretrained/ckpt_swin_t.pth", map_location="cpu")
        model.load_state_dict(checkpoint['model'])
        
        model.head = nn.Linear(768, num_classes)
    else:
        model = FLattenSwinTransformer(  img_size=224, patch_size=4, in_chans=3, num_classes=num_classes,
                                     embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                                     window_size=56, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                                     drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
                                     norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False,
                                     focusing_factor=3, kernel_size=5, attn_type='LLSS')
    # if pretrained:
    #     url = 'https://cloud.tsinghua.edu.cn/f/e1518e76703e4e57a7f2'
    #     # checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
    #     # model.load_state_dict(checkpoint["model"])
    #     checkpoint = torch.load("NAT_FAT/ImageNet_pretrained/ckpt_swin_t.pth", map_location="cpu")
    #     model.load_state_dict(checkpoint['model'])
...
if __name__ == "__main__":
    model = flatten_swin_tiny(num_classes=11)
